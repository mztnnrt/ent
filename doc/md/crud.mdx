...
{label: 'CloudSQL (Google Cloud)', value: 'cloudsql'},
]}>
...
func main() {
	db, err := sql.Open(
		"cloudsqlpostgres",
		"host=<project-id>:<region>:<instance-name> user=<user> dbname=<dbname> password=<password>> sslmode=disable",
	)
	if err != nil {
		log.Fatalf("failed opening connection to postgres: %v", err)
	}
